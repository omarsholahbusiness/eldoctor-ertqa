...
export async function POST(
    ...
    try {
        ...
        // Allow admins to add attachments to any course, or check if user owns the course
        if (user?.role !== "ADMIN") {
...
export async function GET(
    ...
    try {
        ...
        // Allow admins to get attachments from any course, or check if user owns the course
        if (user?.role !== "ADMIN") {
            const courseOwner = await db.course.findUnique({
                where: {
                    id: resolvedParams.courseId,
