...
export async function POST(
    ...
    try {
        ...
        // Allow admins to upload documents to any chapter, or check if user owns the course
        if (user?.role !== "ADMIN") {
...
export async function DELETE(
    ...
    try {
        ...
        // Allow admins to delete documents from any chapter, or check if user owns the course
        if (user?.role !== "ADMIN") {
            const courseOwner = await db.course.findUnique({
                where: {
                    id: resolvedParams.courseId,
